...
const Cadastrar = () => {
    ...
    async function carregarInformacoes()
    {
        if (!id) return;
            try
            {
                ...
                setPreco(jogo.preco || "");
                setClassificacoesSelecionadas(jogo.classificacaoId ? [jogo.classificacaoId] : []);
    ...
    async function salvarJogo(e: React.FormEvent<HTMLFormElement>)
    {
        ...
        try
        {
            ...
            {
                erro("Selecione uma classificação indicativa.");
                return;
            }
            const dados =
            {
                    ...
                    <form id={styles.campo_cadastro} onSubmit={salvarJogo}>
                        ...
                        <div id={styles.formulario}>
                            <div id={styles.esquerda}>
                                ...
                                <div className={styles.campo}>
                                    <label>Gênero</label>
                                    <select multiple size={4} value={generosSelecionados.map(String)} onChange={(e) =>
                                    setGenerosSelecionados(Array.from(e.target.selectedOptions).map((o) => Number(o.value)))}>
                                        {generos.map((g) => (<option key={g.generoId} value={g.generoId}>{g.nome}</option>))}
                                    </select>
                                </div>
                                ...
                                <div className={styles.linha}>
                                    ...
                                    <div className={styles.campo}>
                                        ...
                                        <select value={classificacoesSelecionadas[0]?.toString() || ""} onChange={(e) =>
                                        setClassificacoesSelecionadas([Number(e.target.value),])} required><option value="">Selecione</option>
                                            {classificacoes.map((c) => (<option key={c.classificacaoIndicativaId} value={c.classificacaoIndicativaId}>{c.classificacao}</option>))}
                                        </select>
                                    </div>

                                    <div className={styles.campo}>
                                        <label>Imagem</label>
                                        <input type="file" accept="image/*" onChange={(e) => {if (e.target.files && e.target.files[0]){setImagem(e.target.files[0]);}}}/>
                                    </div>
                                </div>
                        </div>
                        ...
                        <button type="submit" id={styles.botao}>{telaEditar ? "Salvar" : "Cadastrar"}</button>
                    </form>
                </section>
